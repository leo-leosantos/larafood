...
class PlanProfileController extends Controller
{
    ...
    public function attachProfilesPlan(Request $request, $idPlan)
    {
        ...
        if( !$request->profiles || count( $request->profiles) == 0 ){
            return redirect()->back()->with('info','Precisa Escolher pelo menos uma Plano');
    ...
    public function detachProfilePlan($idPlan, $idProfile)
    {
        ...
        return redirect()->route('plans.profiles', $plan->id)->with('message','Permissão desvinculada com sucesso!');
    }
}
